function [F1, F2, params, coefs] = t5_up_control(Y_0, Y_end, t_0, t_end, dt, control_constr2)
% Функция ??
...
for i=1:N
    % ------- -- dy < constr -----------------------------------    
    if ((flag == 0)&&(u_t(i) >= control_constr2))
    % -----------------------------------------------------------------
        if ((i - N_shift) < 1)
            t_left = t_0;
            Y_left = Y_0;
    ...
    % --------- dy > constr ------------------------------------
    if ((flag==1)&&(u_t(i) < control_constr2))    
    % -----------------------------------------------------------------
        if ((i + N_shift) > N + 1)
            t_right = t_end;
            Y_right = Y_end;
...
%              7*d2*(t_1 - t_left).^6/(t_right - t_left)^7);
    ...
    Psi_1 = c0 + c1/(t_end - t_0)*(t_1 - t_0) + c2/(t_end - t_0)^2*(t_1 - t_0).^2 + c3/(t_end - t_0)^3*(t_1 - t_0).^3 +...
            c4/(t_end - t_0)^4*(t_1 - t_0).^4 + c5/(t_end - t_0)^5*(t_1 - t_0).^5 ...
            + d*(t_1 - t_left).^3.*(t_1 - t_right).^3;
        
    dPsi_1 = c1/(t_end - t_0) + 2*c2/(t_end - t_0)^2*(t_1 - t_0) + 3*c3/(t_end - t_0)^3*(t_1 - t_0).^2 + ...
             4*c4/(t_end - t_0)^4*(t_1 - t_0).^3 + 5*c5/(t_end - t_0)^5*(t_1 - t_0).^4 + ...
             d*(3*(t_1 - t_left).^2.*(t_1 - t_right).^3 + 3*(t_1 - t_left).^3.*(t_1 - t_right).^2);
        
    ddPsi_1 = 2*c2/(t_end - t_0)^2 + 6*c3/(t_end - t_0)^3*(t_1 - t_0) + ...
          12*c4/(t_end - t_0)^4*(t_1 - t_0).^2 + 20*c5/(t_end - t_0)^5*(t_1 - t_0).^3 + ...
          d*(6*(t_1 - t_left).*(t_1 - t_right).^3 + 9*(t_1 - t_left).^2.*(t_1 - t_right).^2 + ...
          9*(t_1 - t_left).^2.*(t_1 - t_right).^2 + 6*(t_1 - t_left).^3.*(t_1 - t_right));
   
   u_t = ddPsi_1 - sin(Psi_1);   
      
    if (max(u_t) < control_constr2)
        break;
    end
end
% ddPsi_1 = 2*c2/(t_end - t_0)^2 + 6*c3/(t_end - t_0)^3*(t_1 - t_0) + ...
